...
export const Trigonometric: FunctionComponent = () => {
    ...
    return (
        <EquationsListItem>
            <Head3>
                <Link to={url}>Trigonometric</Link>
            </Head3>
            <Paragraph>
                <Link to={equationsURL([sin], 10)}>y=sin(x);</Link>{' '}
                <Link to={equationsURL([csc], 10)}>y=csc(x);</Link>
                <br />
                <Link to={equationsURL([cos], 10)}>y=cos(x);</Link>{' '}
                <Link to={equationsURL([sec], 10)}>y=sec(x);</Link>
                <br />
                <Link to={equationsURL([tan], 10)}>y=tan(x);</Link>{' '}
                <Link to={equationsURL([cot], 10)}>y=cot(x);</Link>
            </Paragraph>
            <FooterParagraph>
                <Anchor
                    href="https://en.wikipedia.org/wiki/Trigonometric_functions"
                    target="_blank"
